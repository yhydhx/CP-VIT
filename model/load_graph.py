...
def load_graph(name, verbose=False, seed=1):
    if 'raw' in name:
        ...
        directed = False
    filename = cfg.ananimal_brain_netwroks_path + '/' +'{}.txt'.format(name)
    ...
    if not directed:
        adj = np.logical_or(adj.transpose(), adj).astype(int)   #make symmetry

    graph = nx.from_numpy_array(adj, create_using=nx.Graph)  #DiGraph
    if verbose:
        print(type(graph))
